...
pub trait IngressLoadBalancer: Send + Sync {
    ...
    /// # Errors
    ///
    /// Implementations return [`IngressError::InvalidConfig`] when the
    /// node cannot be registered with the underlying infrastructure.
    fn add_node(&self, node: &NodeId) -> Result<(), IngressError> {
        let _ = node;
        Ok(())
    }
    ...
    fn remove_node(&self, node: &NodeId) -> Result<(), IngressError> {
        let _ = node;
        Ok(())
    }
}
