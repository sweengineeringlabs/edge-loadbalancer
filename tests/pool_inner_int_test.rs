...
fn test_pool_inner_round_robin_cycles_across_all_backends() {
    ...
    let pool = LoadbalancerSvc::build_pool(rr_config(3)).expect("pool must build");
    let mut urls: Vec<String> = (0..6)
        .map(|_| LoadbalancerSvc::select(&pool).expect("select must succeed").url)
        .collect();
    urls.dedup();
...
fn test_pool_inner_round_robin_wraps_on_overflow_gracefully() {
    ...
    let pool = LoadbalancerSvc::build_pool(rr_config(2)).expect("pool must build");
    for _ in 0..100 {
        assert!(LoadbalancerSvc::select(&pool).is_ok(), "every select must succeed");
    }
}
...
fn test_pool_inner_weighted_strategy_respects_weight_distribution() {
    ...
    let heavy_count = (0..100)
        .filter(|_| {
            LoadbalancerSvc::select(&pool).map(|b| b.url.contains("heavy")).unwrap_or(false)
        })
        .count();
...
fn test_pool_inner_concurrent_selects_return_valid_backends() {
    ...
    let pool = Arc::new(LoadbalancerSvc::build_pool(config).expect("pool must build"));

    let handles: Vec<_> = (0..8)
        .map(|_| {
            let p = Arc::clone(&pool);
            thread::spawn(move || LoadbalancerSvc::select(&p).is_ok())
        })
        .collect();
