...
use swe_edge_loadbalancer::{LoadbalancerSvc, TenantId, TenantRegistry};
...
fn test_build_tenant_registry_resolves_known_tenants() {
    let reg = LoadbalancerSvc::build_tenant_registry(TOML).unwrap();
...
fn test_build_tenant_registry_malformed_toml_returns_error() {
    let err = LoadbalancerSvc::build_tenant_registry("[tenant.assignments\nacme = ").unwrap_err();
...
fn test_build_tenant_registry_missing_section_returns_empty() {
    ...
    let reg = LoadbalancerSvc::build_tenant_registry("[other]\nfoo = 1\n").unwrap();
