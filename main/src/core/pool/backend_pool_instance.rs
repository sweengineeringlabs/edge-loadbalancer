...
impl BackendPoolInstance {
    ...
    /// # Errors
    ///
    /// Returns `LoadbalancerError::InvalidConfig` when the config fails validation.
    pub(crate) fn build(config: LoadbalancerConfig) -> Result<Self, LoadbalancerError> {
        Self::validate(&config).map_err(LoadbalancerError::InvalidConfig)?;
...
impl BackendPool for BackendPoolInstance {
    fn select(&self) -> Result<Backend, LoadbalancerError> {
        let pi = Self::pool_inner(self).ok_or(LoadbalancerError::NoHealthyBackends)?;
