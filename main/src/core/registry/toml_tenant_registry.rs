...
use crate::api::types::identity::TenantId;
...
impl TomlTenantRegistry {
    ...
    /// # Errors
    ///
    /// Returns `LoadbalancerError::ParseFailed` when the document is not
    /// valid TOML or the section has the wrong shape.
    pub(crate) fn build(toml_str: &str) -> Result<Self, LoadbalancerError> {
        let doc: TenantDocument =
            toml::from_str(toml_str).map_err(|e| LoadbalancerError::ParseFailed(e.to_string()))?;
...
mod tests {
    ...
    fn test_build_malformed_toml_returns_parse_failed_error() {
        ...
        assert!(matches!(err, LoadbalancerError::ParseFailed(_)));
    }
