...
use swe_edge_loadbalancer_egress::{
    BackendConfig, BackendPool, BackendPoolInstance, LoadbalancerConfig, Strategy,
};
...
fn test_pool_inner_round_robin_cycles_across_all_backends() {
    ...
    let pool = BackendPoolInstance::build(rr_config(3)).expect("pool must build");
    let mut urls: Vec<String> =
        (0..6).map(|_| pool.select().expect("select must succeed").url).collect();
    urls.dedup();
...
fn test_pool_inner_round_robin_wraps_on_overflow_gracefully() {
    ...
    let pool = BackendPoolInstance::build(rr_config(2)).expect("pool must build");
    for _ in 0..100 {
        assert!(pool.select().is_ok(), "every select must succeed");
    }
}
...
fn test_pool_inner_weighted_strategy_respects_weight_distribution() {
    ...
    let heavy_count = (0..100)
        .filter(|_| pool.select().map(|b| b.url.contains("heavy")).unwrap_or(false))
        .count();
...
fn test_pool_inner_concurrent_selects_return_valid_backends() {
    ...
    let pool = Arc::new(BackendPoolInstance::build(config).expect("pool must build"));

    let handles: Vec<_> = (0..8)
        .map(|_| {
            let p = Arc::clone(&pool);
            thread::spawn(move || p.select().is_ok())
        })
        .collect();
