...
use swe_edge_loadbalancer_registry::{PoolRegistry, RegistrySvc};
...
fn test_build_pool_registry_starts_empty() {
    let reg = RegistrySvc::build_pool_registry();
...
fn test_register_and_get_shared_pool() {
    let reg = RegistrySvc::build_pool_registry();
    let pool = Arc::new(AutoscaleSvc::build_handler_pool(payment(), None, 4).unwrap());
    RegistrySvc::register_handler_pool(&reg, &payment(), None, pool);
...
fn test_register_tenant_pool_fallback_from_shared() {
    let reg = RegistrySvc::build_pool_registry();
    let shared = Arc::new(AutoscaleSvc::build_handler_pool(payment(), None, 10).unwrap());
    RegistrySvc::register_handler_pool(&reg, &payment(), None, shared);
...
fn test_tenant_override_wins() {
    let reg = RegistrySvc::build_pool_registry();
    let shared: Arc<dyn InstancePool> = Arc::new(AutoscaleSvc::build_handler_pool(payment(), None, 10).unwrap());
    let tenant_pool: Arc<dyn InstancePool> =
        Arc::new(AutoscaleSvc::build_handler_pool(payment(), Some(acme()), 3).unwrap());
    RegistrySvc::register_handler_pool(&reg, &payment(), None, shared);
    RegistrySvc::register_handler_pool(&reg, &payment(), Some(&acme()), Arc::clone(&tenant_pool));
